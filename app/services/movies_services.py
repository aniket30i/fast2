...
async def add_movies(db: AsyncSession, title, year, genres, user_id,director):
    if not title:
        raise ValueError("Movie title cannot be empty")
    if await movies_repository.exists(db, title):
        raise ValueError("Movie already exists")

    return await movies_repository.add_movie(db, title, year, genres, user_id,director)

async def update_movies(db: AsyncSession, movie_id: int, payload: MovieUpdateModel, owner_id: int):
...
async def add_director(db: AsyncSession, name: str):
    if not name:
        raise ValueError("Director name cannot be empty")
    if await movies_repository.director_exists(db, name):
        raise ValueError("Director already exists")
    return await movies_repository.create_director(db, name)

async def get_directors(db: AsyncSession):
    return await movies_repository.get_all_directors(db)
